...
function DesignCard({item}:any) {
  ...
  return (
    <div className='p-5 border rounded-lg'>
      <Image src={item?.imageUrl} alt='image' width={300} height={200}
      ...
      <div className='mt-2'>
        ...
        <div className='flex items-center gap-2 p-2 bg-grey-50 rounded-full '>
     {modelObj && <Image src={modelObj?.icon} alt={modelObj?.modelName ?? '' } width={30} height={30}/> }
     <h2>{modelObj?.name}</h2>
        </div>
        <Link href={'/view-code/'+item?.uid}>
        <Button className=''><Code/>View Code</Button></Link>
